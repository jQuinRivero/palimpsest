...
| Align | 2,000 ms | 8,000 ms | Anchor exact-match block hashes, align only gaps between anchors, apply length-ratio prefilters, score survivors with `rapidfuzz.process.cdist(score_cutoff=...)`, use greedy assignment, then run LIS move detection; disable move detection before refusing. |
...
| `rapidfuzz.process.cdist` | Use batch scoring with `score_cutoff=align_threshold` rather than calling pairwise scorers in Python loops. Candidate pairs below `align_threshold` are discarded in the C++ layer. |
